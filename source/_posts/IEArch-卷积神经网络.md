...
![image-20221010192543050](IEArch-卷积神经网络/image-20221010192543050.webp)
...
![image-20221010193554356](IEArch-卷积神经网络/image-20221010193554356.webp)
...
![image-20221010194040077](IEArch-卷积神经网络/image-20221010194040077.webp)
...
![](IEArch-卷积神经网络/image-20221010211912313.webp)
...
![](IEArch-卷积神经网络/微信图片_20221011114201.webp)

是一个像表达式树的东西，我们需要利用 “对话（session）” 来运行这个结构。

> 计算图包含**数据**（data，也就是各种 Tensor）和**操作**（operation）。一个模型中的每一个数据或者操作都是图中的一个节点，箭头代表了数据流动的方向。上文提到的用 Session 计算的过程实际上就是数据流经整个图，计算出每一个节点的数值。
...
- 独立于模型存在的数据：数据集中的数据需要 “喂给” 网络，包括输入数据、输出端的数据（输出的数据会用于反馈）。这个过程就像数学课本中给函数 f (x) 求值的过程，给 x 不同的值，求得不同的结果。可以想见，我们需要一个类似 “容器” 的对象，每次将数据放进去，然后计算整幅计算图。
- 常量。
- 操作符。
...
这个就是前面说的类似 “容器” 的对象，顾名思义，占位符就是先在模型中” 占位 “，至于这个位置填入的数值，在 Session 运行的时候再指定。我们的输入是 Placeholder
...
![image-20221011150430768](IEArch-卷积神经网络/image-20221011150430768.webp)

准确率变化：

![](IEArch-卷积神经网络/微信图片_20221011162019.webp)

loss 变化

![](IEArch-卷积神经网络/微信图片_20221011162012.webp)

### 5.2 实验结果

2 号实验结果截图：

![](IEArch-卷积神经网络/微信图片_20221011152736.webp)
...
![](IEArch-卷积神经网络/微信图片_20221011152453.webp)
...
![](IEArch-卷积神经网络/微信图片_20221011152426.webp)
