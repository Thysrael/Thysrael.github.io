...
Embedding 的维度通常被称为 $d_{model}$ ，因为博客渲染系统的问题，所以
...
那么 Attention 具体是怎样的呢？首先我们需要先介绍 Attention 的输入。经过 Embedding 过程，每个 token 都是一个 $1 \times d_m$ 维的行向量。我们设序列长度为 $t$ ，那么输入可以被整理成一个 $t \times d_m$ 的矩阵 $X$。
...
当然在有了 $A$ 并不够，我们只是获得了不同 token 之间的联系，但是我们并没有考虑原本 token 的语义，所以我们再将 $A$ 与 $X$ 相乘，那么就可以得到一个新的 $t \times d_{m}$ 的矩阵 $Y$ ，如下所示：
$$
Y = AX = XX^TX
$$
...
- $Q$：Query，即要翻译的中文语句，它的形状是 $t \times d_{in}$ 。
  - $t$ 被理解成语句的长度。
