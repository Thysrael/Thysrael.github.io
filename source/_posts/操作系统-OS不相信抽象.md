...
那么到底是什么样子的？应该这么说**硬件支持了 TLB 的正常功能，而操作系统支持了 TLB 的缺失异常**。在解释这句话前，首先先说一下为啥话题跳到 TLB 了？可以这么理解，其实 CPU 是没办法直接访问物理内存的，他必须经过 TLB。所有的物理页号，其实不来自物理内存中的页表，而是来自 TLB。有了这个概念，其实就可以发现设计的巧妙之处了。
